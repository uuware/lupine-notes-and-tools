...
export const MinePage = async (props: PageProps) => {
  ...
  const renderDashboard = () => {
    ...
    // Calculate simple percentages for the bar
    const notesPct = stats.totalRecords > 0 ? (stats.noteCount / stats.totalRecords) * 100 : 0;
    const diariesPct = stats.totalRecords > 0 ? (stats.diaryCount / stats.totalRecords) * 100 : 0;
    const financePct = stats.totalRecords > 0 ? (stats.financeCount / stats.totalRecords) * 100 : 0;
    ...
    return (
      <div class='mine-dashboard'>
        ...
        <div class='stat-grid'>
          ...
          <div class='stat-card' style={{ borderTopColor: '#eb2f96' }}>
            <div class='stat-icon'>
              <i class='ifc-icon ma-book-outline' style={{ color: '#eb2f96' }} />
            </div>
            <div class='stat-value'>{stats.diaryCount}</div>
            <div class='stat-label'>Diaries</div>
          </div>

          <div class='stat-card' style={{ borderTopColor: '#52c41a' }}>
            <div class='stat-icon'>
              <i class='ifc-icon icon-finance' style={{ color: '#52c41a' }} />
            </div>
        ...
        <div class='usage-bar-container'>
          <div class='usage-bar-labels'>
            ...
            <span>{stats.totalRecords} Items Total</span>
          </div>
          <div class='usage-bar'>
            {stats.totalRecords === 0 && <div class='usage-bar-segment empty' style={{ width: '100%' }}></div>}
            {stats.noteCount > 0 && <div class='usage-bar-segment note' style={{ width: `${notesPct}%` }}></div>}
            {stats.diaryCount > 0 && <div class='usage-bar-segment diary' style={{ width: `${diariesPct}%` }}></div>}
            {stats.financeCount > 0 && (
              <div class='usage-bar-segment finance' style={{ width: `${financePct}%` }}></div>
            )}
          </div>
        </div>
      </div>
